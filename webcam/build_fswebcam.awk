...
{ 
    ...
    print "rm -f movie_1.avi" >> "final_loop.sh"
    ...
    print "ffmpeg -f video4linux2 -s 320x240  -vf \"setpts=(1/"$8")*PTS\" -i /dev/video0 movie_1.avi &" >> "final_loop.sh" 
    #print "pid=$!" >> "final_loop.sh"
    #print "sleep " $10*60 >> "final_loop.sh"    
    print "sleep " $10 >> "final_loop.sh"    
    print "PID=`ps -ef | grep syncapp 'awk {print $2}'`" >> "final_loop.sh"
    print "kill -SIGINT $PID" >> "final_loop.sh"
    print "sleep 1" >> "final_loop.sh"    
    print "kill -9 $PID" >> "final_loop.sh"
    print "killall -9 ffmpeg" >> "final_loop.sh"
    print "mpack -s video movie_1.avi "$2 >> "final_loop.sh"
}
